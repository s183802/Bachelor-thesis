...
int main( int argc, char* argv[])
{
    ...
    std::cout << "js=" << js <<std::endl; // Tilføjet <<"js="

    /////////////////////////////////////////////////////////////////
    dg::Grid1d grid = equations::createGrid( js["grid"], dg::PER);  // Definerer grid variablene som "grid"
    ...
    else if( "soft-step" == init)    // det er denne vi bruger 
    {
        double x_a = js["init"].get("x_a", 0.1).asDouble();
        double n_l = js["init"].get("n_l", 1.0).asDouble();
        double n_r = js["init"].get("n_r", 0.2).asDouble();   // Kan denne eventuelt lede til bedre n_r simuleringer
...
//        GRID = grid.lx();
    ...
    else if( "normal" == init)    // Normal distributed IC
    {
        double Mu = js["init"].get("Mu", 0.).asDouble();           // 
        double sigma = js["init"].get("sigma", 0.4).asDouble();
        double n_floor = js["init"].get("n_floor", 0.01).asDouble();
        double A = js["init"].get("A", 1.).asDouble();
//        double normalx = js["init"].get("normalx", 0.2).asDouble();   // 
        Mu*= grid.lx();      
        sigma*= grid.lx();    
        y0[0][0] = y0[0][1] = dg::evaluate( [&]( double x){ 
                return n_floor + A * exp(-1.0/2.0*pow(((x-Mu)/sigma),2));}, grid);
                //return sqrt(1.0/(2.0*M_PI*pow(sigma,2))) * exp(-1.0/2.0*pow(((x-Mu)/sigma),2));}, grid);
    }
    else if( "wave" == init)
    {
